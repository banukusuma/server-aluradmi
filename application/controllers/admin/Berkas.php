...
* 
*/
class Berkas extends Super_Controller
{
	...
	function index(){
		...
		$this->data['subview'] = 'admin/berkas/index';
	...
	function list_data(){
		...
		foreach ($list as $berkas) {
			$row = array($berkas->id_berkas,$berkas->nama_jurusan,$berkas->nama_kategori,$berkas->nama_alur,$berkas->nama_keterangan,$berkas->nama,  
	...
	function ajax_tambah(){
    	...
    	function edit($id){
    		$this->db->select('b.id_berkas ,b.nama ,a.id_alur,k.id_keterangan, a.id_jurusan, a.id_kategori');
			...
			$this->data['jurusan'] = $this->berkas_m->pilih('jurusan', 'where id_jurusan !=0');	
			$this->data['subview'] = 'admin/berkas/edit';
			$this->load->view('_layout_main', $this->data);
		}
		...
		function update(){
			 ...
			 $isi = array(
				...
				'timestamp' => date('Y-m-d H:i:s'));
			 $validate = $this->berkas_m->validate_kedua;
			$this->form_validation->set_rules($validate);
			if ($this->form_validation->run()) {
		...
		$this->data['kategori'] = $this->berkas_m->pilih('kategori');
		$this->data['jurusan'] = $this->berkas_m->pilih('jurusan', 'where id_jurusan !=0');
		$this->data['subview'] = 'admin/berkas/tambah';
		$this->load->view('_layout_main', $this->data);
		}
}