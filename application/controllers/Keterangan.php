...
* 
*/
class Keterangan extends Admin_Controller
{
	...
	function index(){
		...
		$this->data['subview'] = 'keterangan/index';
		$this->load->view('_layout_main', $this->data);
	}
	function list_data(){
		$list = $this->keterangan_m->get_datatables();
		foreach ($list as $ket) {
			$row = array($ket->id_keterangan,$ket->nama_kategori, $ket->nama_alur,$ket->nama, $ket->keterangan,  $ket->urut, $ket->nama_ruang, 
				'<a class="btn btn-sm btn-success" href="keterangan/edit/'.$ket->id_keterangan.'" title="Edit"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
    ...
    function edit($id){
    	$this->db->select('a.id_kategori,a.id_jurusan, k.id_alur, k.nama, k.id_keterangan, k.keterangan, k.id_ruang');
        $this->db->from('keterangan as k');
        $this->db->join('alur as a', 'a.id_alur = k.id_alur');
        $this->db->where('k.id_keterangan ='.$id);
        $query = $this->db->get()->row();
        $this->data['kategori_data'] = $query;
		...
		$this->data['subview'] = 'keterangan/edit';
		$this->load->view('_layout_main', $this->data);
    ...
    function tambah(){
		...
		$this->data['subview'] = 'keterangan/tambah';
		$this->load->view('_layout_main', $this->data);
    }
    ...
    function ajax_alur(){
    	$id = $this->input->post('kategori');
   		$s_id_j = $this->session->userdata('id_jurusan');
   		$this->data['alur'] = $this->keterangan_m->pilih('alur', 'where id_jurusan ='.$s_id_j.' and id_kategori ='.$id);
   		echo json_encode($this->data);
    }
    ...
    function urut(){
    	$this->data['meta_title'] ='Urutkan Keterangan';
	   	$this->data['kategori'] = $this->keterangan_m->pilih('kategori');
		$this->data['subview'] = 'keterangan/urut';
		$this->load->view('_layout_main', $this->data);
    }
